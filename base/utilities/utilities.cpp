...
namespace VX {
	...
	Vehicle gta_utility::spawn_vehicle(std::uint32_t hash) {
		...
		//*(unsigned short*)GetPointers()->m_set_this_thread_networked = 0x9090; // we set this thread as networked so we can spawn the vehicle / u can add rage classes and use excute under thread instead of this
		...
		//*(unsigned short*)GetPointers()->m_set_this_thread_networked = 0x0574; // We restore it so we don't get detected 
