...
namespace VX {
	FARPROC hooks::get_proc_address(HMODULE hModule, const char* lpProcName) {
		if ((unsigned int)hModule == 0x46b5ab27) { // cockstar
			g_log.send("SHV", std::format("Enable {}", lpProcName).c_str());
			...
			/*else if (strstr(lpProcName, "get_all_vehicles")) {
				return (FARPROC)&script_hook_v::world_get_all_vehicles;
			}
			else if (strstr(lpProcName, "get_all_peds")) {
				return (FARPROC)&script_hook_v::world_get_all_peds;
			}
			else if (strstr(lpProcName, "get_all_objects")) {
				return (FARPROC)&script_hook_v::world_get_all_objects;
			}
			else if (strstr(lpProcName, "get_all_pickups")) {
				return (FARPROC)&script_hook_v::world_get_all_pickups;
			}*/
			else if (strstr(lpProcName, "register_raw_streaming_file")) {
				return (FARPROC)&script_hook_v::register_raw_streaming_file;
			}
		}
		return GetHooking()->m_get_proc_adress.get_original<decltype(&hooks::get_proc_address)>()(hModule, lpProcName);
	}
}