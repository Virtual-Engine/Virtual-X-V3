...
namespace VX
{
	...
	Object SpawnObject(const char* toSpawn) {
		float playerHeading = ENTITY::GET_ENTITY_HEADING(PLAYER::PLAYER_ID());

		float xVect = 2.f * sin(degToRad(playerHeading)) * -1.0f;
		float yVect = 2.f * cos(degToRad(playerHeading));

		DWORD model = MISC::GET_HASH_KEY(toSpawn);
		Object object;
		Vector3 pos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1);
		if (STREAMING::IS_MODEL_IN_CDIMAGE(model)) {
			STREAMING::REQUEST_MODEL(model);
			object = OBJECT::CREATE_OBJECT(model, pos.x, pos.y, pos.z, 0.f, 1, 1);
		}
	...
	const char* m_objectmodels[9465] = {
		"02gate3_l",
