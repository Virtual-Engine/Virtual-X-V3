...
namespace VX
{
	...
	public:
		void Init();
		bool cursor_at_pos(Vector2 const& boxCentre, Vector2 const& boxSize);
		Vector2 get_cursor_location();
		void MousseInput();
		bool mouse_activate = false;
		const char* ToggleMousseOn = "Mousse On";
		const char* ToggleMousseOff = "Mousse Off";
	};

	Mousse* GetMousse();

	const std::map<int, std::string> VirtualKeyToString
	{
