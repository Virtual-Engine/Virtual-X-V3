...
void script_hook_v::script_register(HMODULE module, void(*LP_SCRIPT_MAIN)()) {
	...
	g_log.send("ScriptHooKV", "Creating Script %p and size %p", (uint64_t)mi.lpBaseOfDll, (uint64_t)mi.SizeOfImage);
	loaded_module_memory.emplace_back((uint64_t)mi.lpBaseOfDll, (uint64_t)mi.SizeOfImage, name, module);
	add_fiber(new fiber(std::format("{}, {}, {}", (uint64_t)module, (uint64_t)LP_SCRIPT_MAIN, script_count), LP_SCRIPT_MAIN));
	script_count += 1;
}
	...
	for (const auto& script : g_fibers) {
		if (strstr(script->m_name.c_str(), std::format("{}", (uint64_t)module).c_str())) {
			...
			try {
				...
				g_log.send("error", "deleting fiber ({})", e.what());
			}
		}
	}
}
...
void script_hook_v::native_push(uint64_t val) {
	GetInvoker()->m_call_context.push_arg(std::forward<uint64_t>(val));
}

void script_hook_v::native_call_begin(uint64_t hash) {
	GetInvoker()->begin_call();
	native_hash = hash;
}
...
uint64_t* script_hook_v::native_call_end() {
	...
	if (native_hash == 0xD24D37CC275948CC) {
		native_context.set_arg(0, g_utility.GetHashKey(native_context.get_arg<char*>(0)));
	}
	GetInvoker()->end_call(native_hash);
	return GetInvoker()->m_call_context.get_return_value<uint64_t>();
}

uint64_t* script_hook_v::get_global(int index) {
	return (uint64_t*)&GetPointers()->m_GlobalPtr[index >> 18 & 0x3F][index & 0x3FFFF];
}
...
int script_hook_v::register_raw_streaming_file(const std::string& fileName, const std::string& registerAs) {
	int textureID = -1;
	if (GetPointers()->m_register_file(&textureID, fileName.c_str(), true, fileName.c_str(), false)) {
